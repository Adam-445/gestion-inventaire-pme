...
class GestionCategoriesFrame(tk.Frame):
    ...
    def create_widgets(self):
        # Titre
        tk.Label(self, text="Gestion des Catégories", font=("Arial", 16, "bold")).pack(
            pady=10
        )
        ...
        tk.Label(form_frame, text="Nom de la Catégorie:").grid(
        ...
        self.tree = ttk.Treeview(
            list_frame, columns=("ID", "Nom", "Description"), show="headings"
        )
        ...
        tk.Button(action_frame, text="Modifier", command=self.open_edit_dialog).pack(
            side="left", padx=5
        )
        tk.Button(
            action_frame,
        ...
        try:
            ...
            for cat in categories:
                self.tree.insert(
                    "",
                    tk.END,
                    values=(cat["categorie_id"], cat["nom"], cat["description"]),
                )
        except Exception as e:
            messagebox.showerror(
                "Erreur DB", f"Impossible de charger les catégories:{e}"
            )
    ...
    def add_category(self):
        ...
        if not nom:
            messagebox.showwarning(
                "Entrée Invalide", "Le nom de la catégorie ne peut pas être vide."
            )
            return
        try:
            new_id = creer_categorie(nom, description)
            messagebox.showinfo(
                "Succès", f"Catégorie '{nom}' (ID: {new_id}) ajoutée avec succès."
            )
            self.load_categories()
            ...
            messagebox.showerror(
                "Erreur d'Ajout", f"Erreur lors de l'ajout de la catégorie : {e}"
            )
    ...
    def confirm_delete(self):
        ...
        if messagebox.askyesno(
            ...
            try:
                supprimer_categorie(cat_id)
                messagebox.showinfo("Succès", f"Catégorie '{nom}' supprimée.")
                self.load_categories()
            except Exception as e:
                messagebox.showerror(
                    "Erreur de Suppression",
                    f"Impossible de supprimer la catégorie : {e}",
                )
    ...
    def open_edit_dialog(self):
        ...
        def save_changes():
            ...
            try:
                ...
                success = modifier_categorie(
                    int(selected_cat["id"]), new_name, new_desc
                )
                if success:
        ...
        tk.Button(dialog, text="Sauvegarder", command=save_changes).grid(
